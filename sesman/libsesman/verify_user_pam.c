              ...
              struct pam_response *reply)
{
    ...
    {
        error = scp_send_prompt_request(cf_data->scp_trans, msg->msg, 1);
             ...
             struct pam_response *reply)
{
    ...
    {
        error = scp_send_prompt_request(cf_data->scp_trans, msg->msg, 2);
        LOG_DEVEL(LOG_LEVEL_INFO, "scp_send_prompt_request %d", error);
        if (error == 0)
        {
            error = scp_msg_in_wait_available(cf_data->scp_trans);
               ...
               struct pam_response *reply)
{
    ...
    if (cf_data != NULL && cf_data->scp_trans != NULL)
    {
        error = scp_send_prompt_request(cf_data->scp_trans, msg->msg, 3);
        LOG_DEVEL(LOG_LEVEL_INFO, "scp_send_prompt_request %d", error);
        if (error != 0)
        {
            LOG(LOG_LEVEL_ERROR, "scp_send_prompt_request failed %d", error);
               ...
               struct pam_response *reply)
{
    ...
    if (cf_data != NULL && cf_data->scp_trans != NULL)
    {
        error = scp_send_prompt_request(cf_data->scp_trans, msg->msg, 4);
        LOG_DEVEL(LOG_LEVEL_INFO, "scp_send_prompt_request %d", error);
        if (error != 0)
        {
            LOG(LOG_LEVEL_ERROR, "scp_send_prompt_request failed %d", error);
