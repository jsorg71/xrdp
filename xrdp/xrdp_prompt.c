...
/* id 4 to 8 are the labels for text */
...
xrdp_prompt_create(struct xrdp_bitmap *wnd, struct xrdp_bitmap **aprompt_wnd)
{
    ...
    prompt_wnd = xrdp_bitmap_create(400, 300, wnd->wm->screen->bpp,
                                    WND_TYPE_WND, wnd->wm);
    ...
    edit_width = prompt_wnd->width - 30;
    ...
    edit->left = 15;
    edit->top = prompt_wnd->height - but_height - edit_height - 30;
    ...
    can_but->left = prompt_wnd->width - but_width - 15;
    can_but->top = prompt_wnd->height - but_height - 15;
    ...
    ok_but->left = can_but->left - but_width - 15;
    ok_but->top = prompt_wnd->height - but_height - 15;
...
/*****************************************************************************/
static int
xrdp_prompt_add_prompt2(struct xrdp_bitmap *prompt_wnd, const char *line_text)
{
    struct xrdp_bitmap *line_wnd;
...
/*****************************************************************************/
static int
xrdp_prompt_add_prompt1(struct xrdp_bitmap *prompt_wnd, const char *line_text,
                        struct xrdp_painter *p)
{
    int text_width;
    int label_width;
    ...
    {
        return xrdp_prompt_add_prompt2(prompt_wnd, line_text);
    }
    // have to wrap
    dest_idx = 0;
    save_dest_idx = -1;
    ...
    while (src[0] != 0)
    {
        ...
        if (xrdp_painter_text_width(p, dest) > label_width)
        {
            if (save_dest_idx == -1)
            {
                xrdp_prompt_add_prompt2(prompt_wnd, dest);
                dest[0] = 0;
                dest_idx = 0;
            }
            else
                       ...
                       int flags)
{
    ...
    while (prompt[jndex] != 0)
    {
        ...
        {
            xrdp_prompt_add_prompt1(wnd, line, p);
            line[0] = 0;
            index = 0;
            jndex++;
            continue;
